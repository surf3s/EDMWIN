...
ElseIf SetUpTypes(3) Then
    ...
    Case 0
        ...
        Call sethortangle("0.0000", 0, 0, 0)
        Call takeshot_nostation(AskForPrism)
        If Cancelling Or errorcode <> 0 Then
            cmdRecord(Index).Enabled = True
            mdiMain.StatusBar.Panels(6).Visible = False
    ...
    Case 1
        ...
        currentyp = edmshot.y
        currentzp = edmshot.z
        stationx = datum2x - currentxp
        stationy = datum2y - currentyp
        
        DistanceAB = Sqr((datum1x - datum2x) ^ 2 + (datum1y - datum2y) ^ 2)
    
        If DistanceAB = 0 Then
            MsgBox "Error: First and second shot must have different coordinates.", vbInformation
            mdiMain.StatusBar.Panels(6).Visible = False

            Exit Sub
        End If
            
        DistanceAS = Sqr(firstxp ^ 2 + firstyp ^ 2)
        DistanceBS = Sqr(currentxp ^ 2 + currentyp ^ 2)
        
        SideOpposite = datum2y - datum1y
        SideAdjacent = datum2x - datum1x
        
        CosAngle = SideAdjacent / DistanceAB
        SinAngle = Sqr(1 - CosAngle ^ 2)
        
        ForesightAB = Atn(SinAngle / CosAngle) * 57.2958
        
        If datum1y <= datum2y And datum1x <= datum2x Then
            ForesightAB = 270 - ForesightAB
        ElseIf datum1y <= datum2y And datum1x >= datum2x Then
            ForesightAB = 90 - ForesightAB
        ElseIf datum1y >= datum2y And datum1x <= datum2x Then
            ForesightAB = 270 + ForesightAB
        ElseIf datum1y >= datum2y And datum1x >= datum2x Then
            ForesightAB = 90 + ForesightAB
        ...
        CosAngle = (DistanceAS ^ 2 - DistanceBS ^ 2 - DistanceAB ^ 2) / (-2 * DistanceBS * DistanceAB)
        
        If CosAngle < 0 Or CosAngle > 1 Then
            MsgBox "Calculation error: The datums selected and the points recorded do not match.  Verify that you have used the correct datums.", vbInformation
            mdiMain.StatusBar.Panels(6).Visible = False
            Exit Sub
        End If
        
        SinAngle = (1 - CosAngle ^ 2)
        AngleDifference = Atn(SinAngle / CosAngle) * 57.2958
        AngleDifference = (Atn(-CosAngle / Sqr(-CosAngle * CosAngle + 1)) + 2 * Atn(1)) * 57.2958
        If AngleDifference < 0 Then AngleDifference = 180 + AngleDifference
    
        If edmshot.hangle < 180 Then
            horizontalchange = -1
        Else
            horizontalchange = 1
        End If
        
        ForesightBS = horizontalchange * AngleDifference + ForesightAB
        If ForesightBS > 360 Then ForesightBS = ForesightBS - 360
        If ForesightBS < 0 Then ForesightBS = 360 + ForesightBS
    
        ForesightSB = ForesightBS + 180
        If ForesightSB > 360 Then ForesightSB = ForesightSB - 360
    
        currentstationx = datum2x - DistanceBS * Sin(ForesightSB / 57.2958)
        currentstationy = datum2y - DistanceBS * Cos(ForesightSB / 57.2958)
        currentstationz = datum2z - currentzp
        
        txtX(0).Text = Format(currentstationx, "######0.000")
        txtY(0).Text = Format(currentstationy, "######0.000")
        txtZ(0).Text = Format(currentstationz, "######0.000")
        txtReferenceAngle = ForesightBS
        
        Call conv_angle_to_degminsec(ForesightSB, degrees, minutes, seconds)
        Call sethortangle(txtReferenceAngle, degrees, minutes, seconds)
...
Private Sub lblInstructions_Click(Index As Integer)
